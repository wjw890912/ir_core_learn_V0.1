...
{    
    ...
    {
			outp32(&PWMA->CCR0, inp32(&PWMA->CCR0) & 0x000F001F);
    ...
    {
			outp32(&PWMA->CCR0, inp32(&PWMA->CCR0) & 0x001F000F);
    ...
    {
			outp32(&PWMA->CCR2, inp32(&PWMA->CCR2) & 0x000F001F);
    ...
    {
			outp32(&PWMA->CCR2, inp32(&PWMA->CCR2) & 0x001F000F);
...
{    
	...
	if (u32CapIntFlag0 & BIT4) 
    {
			outp32(&PWMB->CCR0, inp32(&PWMB->CCR0) & 0x000F001F);
	...
	if (u32CapIntFlag0 & BIT20) 
    {
			outp32(&PWMB->CCR0, inp32(&PWMB->CCR0) & 0x001F000F);
	...
	if (u32CapIntFlag1 & BIT4) 
    {
			outp32(&PWMB->CCR2, inp32(&PWMB->CCR2) & 0x000F001F);
	...
	if (u32CapIntFlag1 & BIT20) 
    {
			outp32(&PWMB->CCR2, inp32(&PWMB->CCR2) & 0x001F000F);
...
void DrvPWM_ClearCaptureIntStatus(uint8_t u8Capture, uint8_t u8IntType)
{
	...
	{
			case DRVPWM_CAP0:
				(u8IntType == DRVPWM_CAP_RISING_FLAG)? (outp32(&PWMA->CCR0, inp32(&PWMA->CCR0) & 0x000F004F)) : (outp32(&PWMA->CCR0, inp32(&PWMA->CCR0) & 0x000F008F));
				break;
			case DRVPWM_CAP1:
				(u8IntType == DRVPWM_CAP_RISING_FLAG)? (outp32(&PWMA->CCR0, inp32(&PWMA->CCR0) & 0x004F000F)) : (outp32(&PWMA->CCR0, inp32(&PWMA->CCR0) & 0x008F000F));
				break;
			case DRVPWM_CAP2:
				(u8IntType == DRVPWM_CAP_RISING_FLAG)? (outp32(&PWMA->CCR2, inp32(&PWMA->CCR2) & 0x000F004F)) : (outp32(&PWMA->CCR2, inp32(&PWMA->CCR2) & 0x000F008F));
				break;																	  
			case DRVPWM_CAP3:
				(u8IntType == DRVPWM_CAP_RISING_FLAG)? (outp32(&PWMA->CCR2, inp32(&PWMA->CCR2) & 0x004F000F)) : (outp32(&PWMA->CCR2, inp32(&PWMA->CCR2) & 0x008F000F));
				break;
			case DRVPWM_CAP4:
				(u8IntType == DRVPWM_CAP_RISING_FLAG)? (outp32(&PWMB->CCR0, inp32(&PWMB->CCR0) & 0x000F004F)) : (outp32(&PWMB->CCR0, inp32(&PWMB->CCR0) & 0x000F008F));
				break;
			case DRVPWM_CAP5:
				(u8IntType == DRVPWM_CAP_RISING_FLAG)? (outp32(&PWMB->CCR0, inp32(&PWMB->CCR0) & 0x004F000F)) : (outp32(&PWMB->CCR0, inp32(&PWMB->CCR0) & 0x008F000F));
				break;
			case DRVPWM_CAP6:
				(u8IntType == DRVPWM_CAP_RISING_FLAG)? (outp32(&PWMB->CCR2, inp32(&PWMB->CCR2) & 0x000F004F)) : (outp32(&PWMB->CCR2, inp32(&PWMB->CCR2) & 0x000F008F));
				break;																	  
			case DRVPWM_CAP7:
				(u8IntType == DRVPWM_CAP_RISING_FLAG)? (outp32(&PWMB->CCR2, inp32(&PWMB->CCR2) & 0x004F000F)) : (outp32(&PWMB->CCR2, inp32(&PWMB->CCR2) & 0x008F000F));
				break;
	} 
}
...
uint32_t DrvPWM_SetTimerClk(uint8_t u8Timer, S_DRVPWM_TIME_DATA_T *sPt)
{
	...
	switch (u8Timer)
	{
        ...
        case DRVPWM_CAP0:
            outp32(&PWMA->CCR0, (inp32(&PWMA->CCR0) & CCR_MASK) | (sPt->i32Inverter));
            break;
        case DRVPWM_CAP1:
            outp32(&PWMA->CCR0, (inp32(&PWMA->CCR0) & CCR_MASK) | ((sPt->i32Inverter)<<16));
            break;
        case DRVPWM_CAP2:
            outp32(&PWMA->CCR2, (inp32(&PWMA->CCR2) & CCR_MASK) | (sPt->i32Inverter));
            break;
        case DRVPWM_CAP3:
            outp32(&PWMA->CCR2, (inp32(&PWMA->CCR2) & CCR_MASK) | ((sPt->i32Inverter)<<16));
            break;
        case DRVPWM_CAP4:
            outp32(&PWMB->CCR0, (inp32(&PWMB->CCR0) & CCR_MASK) | (sPt->i32Inverter));
            break;
        case DRVPWM_CAP5:
            outp32(&PWMB->CCR0, (inp32(&PWMB->CCR0) & CCR_MASK) | ((sPt->i32Inverter)<<16));
            break;
        case DRVPWM_CAP6:
            outp32(&PWMB->CCR2, (inp32(&PWMB->CCR2) & CCR_MASK) | (sPt->i32Inverter));
            break;
        case DRVPWM_CAP7:
            outp32(&PWMB->CCR2, (inp32(&PWMB->CCR2) & CCR_MASK) | ((sPt->i32Inverter)<<16));
            break;
	} 
